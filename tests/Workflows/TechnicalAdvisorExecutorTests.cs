...
public class TechnicalAdvisorExecutorTests
{
    [Fact]
    public async Task HandleAsync_AnswersTechnicalQuestion()
    {
        ...
        // Setup LLM to return technical answer
        llm.Setup(l => l.GetUpdatedFileAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
            .ReturnsAsync("{\"Question\":\"Which framework should we use?\",\"Answer\":\"Use ASP.NET Core for the web API\",\"Reasoning\":\"Based on playbook constraints and best practices\"}");
        ...
        var classificationResult = "{\"Classification\":{\"Question\":\"Which framework should we use?\",\"Type\":\"Technical\",\"Reasoning\":\"Framework selection question\"}}";
        ...
        var executor = new TechnicalAdvisorExecutor(workContext, config.Workflow);
        ...
        Assert.Contains("ASP.NET Core", storedAnswer);
